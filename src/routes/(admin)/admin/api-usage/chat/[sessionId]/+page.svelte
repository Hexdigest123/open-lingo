...
<div class="mx-auto max-w-4xl p-4">
	<a href="/admin/api-usage" class="mb-4 inline-block text-primary hover:underline">
		&larr; {m["admin.apiUsage.chatSession.back"]()}
	</a>

	<!-- Session Info -->
	<div class="bg-card-bg mb-6 rounded-xl p-4">
		<h1 class="mb-2 text-xl font-bold text-text-light">
			{data.session.title || m["admin.apiUsage.chatSession.untitled"]()}
		</h1>
		<div class="grid gap-2 text-sm text-text-muted sm:grid-cols-2">
			<div>
				<span class="font-medium">{m["admin.apiUsage.chatSession.user"]()}</span>
				{data.session.userDisplayName} ({data.session.userEmail})
			</div>
			<div>
				<span class="font-medium">{m["admin.apiUsage.chatSession.mode"]()}</span>
				<span
			...
			<div>
				<span class="font-medium">{m["admin.apiUsage.chatSession.created"]()}</span>
				{formatDate(data.session.createdAt)}
			</div>
			<div>
				<span class="font-medium">{m["admin.apiUsage.chatSession.lastUpdated"]()}</span>
	...
	<div class="space-y-4">
		<h2 class="text-lg font-bold text-text-light">
			{m["admin.apiUsage.chatSession.conversation"]({ count: data.messages.length })}
		</h2>
		...
		{#each data.messages as message}
			...
			<div class="rounded-xl p-4 {bgClass}">
				<div class="mb-2 flex items-center justify-between">
					...
					>
						{message.role === 'assistant'
							? m["admin.apiUsage.chatSession.aiTutor"]()
							: message.role === 'user'
								? m["admin.apiUsage.chatSession.userRole"]()
								: m["admin.apiUsage.chatSession.system"]()}
					</span>
		...
		{#if data.messages.length === 0}
			<div class="bg-card-bg rounded-xl p-8 text-center text-text-muted">
				{m["admin.apiUsage.chatSession.noMessages"]()}
			</div>
		{/if}
	</div>
</div>
